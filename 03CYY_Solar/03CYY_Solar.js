...
//行星X的
var hourOfDayX = 0.0;		// 行星X一天中的小时数，初始为0
var dayOfYearX = 0.0;		// 行星X一年中的天数，初始为0
...
window.onload = function main(){
	...
	matProj = perspective(30.0, 		// 垂直方向视角(角度越大，透视效果越强（鱼眼镜头感觉）)
...
function animation(){
	...
	if (runAnimation) {	// 如果动画开启
        ...
        hourOfDay += hours;
        dayOfYear += hours / 24.0;    //地球24h一天

		hourOfDayX += hours;
        dayOfYearX += hours / 36.0;   //行星X系统36h一天
...
function drawSolarSystem(mvp){
	...
	//控制两个小太阳旋转
	mvp = mult(mvp,rotateY(3600.0*dayOfYear/365.0));
	
	/*绘制左方小太阳*/
	mvpStack.push(mvp); // 将MVP矩阵压进栈
	mvp = mult(mvp, translate(-0.7,0,0));  //向左平移，平移量决定小太阳系中心的距离
	...
	/*绘制右方小太阳*/
	mvp = mult(mvp, translate(0.7,0,0));  //向右平移
	mvp = mult(mvp,rotateX(90));  //调整南北极为y轴方向
	mvp = mult(mvp, scale(0.5, 0.5, 0.5)); // 控制太阳大小的缩放变换
    // 太阳直接画在原点，无须变换，用一个黄色的球体表示
	gl.uniformMatrix4fv(u_MVPMatrix, false, flatten(mvp)); // 传模视投影矩阵
    ...
    /*对地球系统定位，绕太阳放置它*/
	mvpStack.push(mvp);
    mvp = mult(mvp, rotateY(360.0 * dayOfYear / 365.0));  // 用dayOfYear来控制其绕太阳的旋转
    mvp = mult(mvp, translate(4.0, 0.0, 0.0));  // 用于控制地球和太阳之间距离的平移变换
	mvp = mult(mvp, rotateY(-360.0 * dayOfYear / 365.0)); //公转抵消倾斜角度的影响
	mvp = mult(mvp, rotateZ(-40));  //倾斜角度
	drawEarthSystem(mvp);  	// 绘制地球系统
	mvp = mvpStack.pop();

	
	/*绘制行星X系统*/
	mvpStack.push(mvp);
	mvp = mult(mvp,rotateY(3600.0*dayOfYearX/300.0));  //用dayOfYearX来控制其绕太阳系中心旋转
	mvp = mult(mvp, translate(2.5, 0.0, 0.0));   //用于控制行星X系统和太阳系中心之间距离的平移变换
	drawStarXystem(mvp);  // 绘制行星X系统
	mvp = mvpStack.pop();
...
// 绘制行星X系统
// 参数为模视投影矩阵
function drawStarXystem(mvp){
	
	mvpStack.push(mvp);
	// 行星X自转，用hourOfDay进行控制
	mvp = mult(mvp, rotateY(360.0 * hourOfDayX / 36.0));
	mvp = mult(mvp,rotateX(90));  //调整南北极为y轴方向
	mvp = mult(mvp, scale(0.5, 0.5, 0.5));// 控制行星X大小的缩放变换
	// 画一个球
	gl.uniformMatrix4fv(u_MVPMatrix, false, flatten(mvp)); // 传模视投影矩阵
	gl.uniform3f(u_Color, 0.1, 0.9, 0.9 );  
	gl.drawArrays(gl.LINES, 0, numVertices);
	mvp = mvpStack.pop();

	/*画同步卫星*/
	mvpStack.push(mvp);
  	mvp = mult(mvp, rotateY(360.0 * hourOfDayX / 36.0)); // 旋转速度和行星X自转速度一致
    mvp = mult(mvp, translate( 0.7, 0.0, 0.0 ));
	mvp = mult(mvp,rotateX(90));  //调整南北极为y轴方向
	mvp = mult(mvp, scale(0.1, 0.1, 0.1));// 控制同步卫星大小的缩放变换
	gl.uniformMatrix4fv(u_MVPMatrix, false, flatten(mvp)); // 传模视投影矩阵
	gl.uniform3f(u_Color, 0.8, 0.3, 0.5 ); 
	gl.drawArrays(gl.LINES, 0, numVertices);
	mvp = mvpStack.pop();
	
	/*绘制逆向卫星*/
	mvp = mult(mvp, rotateY(-8 * 360.0 * dayOfYearX / 36.0)); //用dayOfYearX进行控制
    mvp = mult(mvp, translate( 0.95, 0.0, 0.0 ));
	mvp = mult(mvp,rotateX(90));  //调整南北极为y轴方向
	mvp = mult(mvp, scale(0.14, 0.14, 0.14));// 控制逆向卫星大小的缩放变换
	//mvp = mult(mvp, scale(2, 2, 2));
	gl.uniformMatrix4fv(u_MVPMatrix, false, flatten(mvp)); // 传模视投影矩阵
	gl.uniform3f(u_Color, 0.95, 0, 0 ); 
	gl.drawArrays(gl.LINES, 0, numVertices);
...
function drawMoonSystem(mvp){
    ...
    mvp = mult(mvp, translate( 0.2, 0.0, 0.0 ));  //控制月球卫星与月球之间的距离
	mvp = mult(mvp,rotateX(90));  //调整南北极为y轴方向
	mvp = mult(mvp, scale(0.05, 0.05, 0.05));
	gl.uniformMatrix4fv(u_MVPMatrix, false, flatten(mvp)); // 传模视投影矩阵
...
function drawEarthSystem(mvp){
    ...
    mvp = mult(mvp, translate( 0.7, 0.0, 0.0 ));  // 用于控制地月距离的平移变换
	drawMoonSystem(mvp);
